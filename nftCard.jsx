export const NFTCard = ({nft}) => {
    return (
        <div className="w-1/4 flex flex-col">
            ...
            <div className="flex flex-col y-gap-2 px-2 py-3 bg-slate-100 rounded-b-md h-100">
                <h2 className="text-xl text-gray-800">{nft.title}</h2>
                <p className="text-gray-600">{nft.id.tokenId.substr(nft.id.tokenId.length - 4)}</p>
                <p id="copy" className="text-gray-600">{`${nft.contract.address.substr(0, 4)}...${nft.contract.address.substr(nft.contract.address.length - 4)}`}</p>
            </div>
